2022-0418-01)
  6) WIDTH_BUCKET(n,min,max,b)
    - min에서 max값의 범위를 b개의 구간으로 나누고 주어진 값 n이 속한 구간의
      인덱스 값을 반환.
    - max 값은 구간에 포함되지 않으며, min보다 작은 n값은
      0구간 max보다 큰 값은 b+1구간 인덱스를 반환.
사용예)
...
사용예)회원테이블에서 1000~6000 마일리지르 6개의 구간으로 나누었을때 각 회원들의
      마일리지에 따른 등급을 구하여 출력하시오
      Alias는 회원번호, 회원명,마일리지,등급
      단, 등급은 마일리지가 6000초과한 회원부터 1등급에서 마지막 등급으로 분류하시오.
...
사용예) SELECT ADD_MONTHS(SYSDATE, 120) FROM DUAL;
 ...
 3) NEXT_DAY(d, c)
   . 주어진 날짜 다음에 처음 만나는 c요일의 날짜를 반환. -- 돌아오는 요일.
...
5) EXTRACT(fmt FROM d) -- 추출하다, 뽑아내다,
   - 주어진 날짜자료 d에서 fmt(Format string:형식문자열)로 제시된 값을 반환
   - fmt는 YEAR, MONTH, DAY, HOUR, MINUTE, SECOND 중 "하나" -- 둘 이상 사용 불가.
   - 결과는 숫자자료이다.

사용예)회원테이블에서 이번달에 생일이 있는 회원을 조회하시오.
      Alias는 SELECT BUY_DATE AS 날짜,
...
사용예)오늘이 2020년 4월 18일이라고 가정할때
  ...
  SELECT EMPLOYEE_ID AS 사원번호,
         ...
         LPAD((EXTRACT(YEAR FROM TO_DATE('20200418'))-
           EXTRACT(YEAR FROM HIRE_DATE)||'년'),9,' ') AS 근속년수,
         SALARY AS 급여
    FROM HR.employees
