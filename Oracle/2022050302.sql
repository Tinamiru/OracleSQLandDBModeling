...
사용예) 다음 사원을 사원테이블에 저장하는 익명블록을 작성하시오
        저장하기전에 해당 사원 이름으로 직원의 존재 유무를 판단하여
        같은 이름이 있으면 갱신을, 없으면 삽입을 수행하시오.
        사원번호는 가장 큰 사원번호 다음번호로 지정한다.
...
사용예)회원테이블에서 마일리지를 조회하여
 ...
 DECLARE
   CURSOR CUR_MEM02  IS  -- 결과집합. 뷰와 같다
     SELECT MEM_NAME,MEM_MILEAGE FROM MEMBER; -- 커서를 사용하여 아래의 셀렉트문을 변수로 지정.
   V_RES VARCHAR2(200);
 BEGIN
